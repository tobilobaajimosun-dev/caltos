...
- `.dark` overrides the surface/text/status tokens above — write new component styles against tokens (not hardcoded hex) so they adapt automatically. The whole app (not just the shell) is now audited to use tokens instead of hardcoded hex for surfaces/borders/text — a few colors are intentionally left as fixed hex because they must NOT flip with the theme: toggle switch knobs (always white, regardless of track color), the inline-file "paper" icon gradient, and semantic status/brand accent colors (success green, error red, warning amber, brand blue) which are tinted badges/icons meant to read the same in both themes.
- **Gotcha:** `ThemeService` is `providedIn: 'root'` but Angular only constructs a root service when something actually injects it — on public routes with no shell (`/login`, `/onboarding`, etc.) nothing did, so the persisted theme silently failed to apply on a fresh load of those pages. Fixed by injecting `ThemeService` in `AppComponent` (`src/app/app.ts`) itself, which renders on every route. If you add another root-level service that must apply globally regardless of route, inject it there too.
- The dark-mode toggle UI lives in the header only (a sun/moon icon button) — do not add a duplicate toggle switch elsewhere (removed from the sidebar and from the header's user-menu dropdown to avoid triplication).
...
- Collapsed (icon-only) sidebar nav items show their label in an `app-tooltip` (`position="right"`) on hover/focus. The sidebar's own scroll region had to move from the `<nav>` element to just the nav-list (`overflow: visible` on `.sidebar`, `overflow-y: auto` on `.nav-list`) — otherwise `overflow-x: hidden` on the sidebar clipped the tooltip popout for collapsed icons.
- The header has a direct sun/moon icon button for dark mode (the only toggle in the app — not duplicated in the sidebar or the user-menu dropdown) plus a **balance pill** next to the notification bell (`account.balance()`). There is no wallet-balance or account-number display in the sidebar — that's the header's job now; the Home dashboard's Wallet card is the only other place account info shows.
- `AccountService` (`src/app/shared/services/account.service.ts`) is the single source of truth for `balance`, `accountNumber`, and `bankName` — don't hardcode these elsewhere.
